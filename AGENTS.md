...
- Display: the quota section always retains its selector, including when the user selects nothing. A cold `.noData` stays visible as 「正在读取订阅配额…」 while that provider's refresh is in flight (both legs now have real latency); two selected providers render side-by-side and one renders full-width. A selected Cursor remains visible with 「已识别 Cursor · 等待官方配额接口」 even though it intentionally has no meters. Cards accept provider-neutral meters, render at most two, and summarize the rest as 「另有 N 项」.
