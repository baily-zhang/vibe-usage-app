...
- `QuotaProductRegistry` lists Kimi Code, ZCode, and Grok (Cursor) in the selector and can recognize their ordinary local app/config/CLI presence without opening those files. They remain visibly pending and non-selectable until a verified, versioned CLI quota protocol adapter exists; do not make an integration appear ready by scraping browser cookies, another app's keychain, network traffic, or UI.
