...
- The dashboard and Settings share one ordered, persisted product selection (`selectedQuotaProductIds`), capped at two. First launch selects locally detected ready products; a one-time migration preserves the legacy Codex/Claude toggles exactly. `quotaSelectionInitialized` distinguishes a fresh install from a user intentionally selecting nothing. Local discovery never reads credentials or reaches the network, and a product discovered after initialization never replaces the user's choices. `codexRateLimitEnabled` / `claudeRateLimitEnabled` are compatibility projections over this selection while their native adapters remain in place.
- `QuotaProductRegistry` lists Kimi Code, ZCode, and Grok (Cursor) in the selector and recognizes ordinary local app/config/CLI presence without opening those files. Kimi and ZCode use the CLI's schema-versioned quota contract; Grok (Cursor) stays visibly pending until an official/local stable protocol exists. Never make an integration appear ready by scraping browser cookies, another app's keychain, network traffic, or UI.
- Kimi reads the official Kimi CLI OAuth file (`$KIMI_SHARE_DIR/credentials/kimi-code.json`, otherwise `~/.kimi/credentials/kimi-code.json`) through the shared CLI and calls the official `/coding/v1/usages` endpoint. The shared CLI is the sole Vibe-side owner of Kimi refresh behavior: when the access token is near expiry or a usage request returns 401, it uses Kimi's standard OAuth refresh grant, serializes Vibe refreshes across processes, re-checks concurrent Kimi CLI rotation, and atomically writes the rotated credential at mode `0600`. No token reaches app output, diagnostics, quota cache, or upload state. ZCode accepts only a user-entered Z.ai API key, stored in Vibe Usage's own Keychain item and injected into the selected quota subprocess as `Z_AI_API_KEY`; it never reads ZCode's OAuth database. Normalized CLI quota cache entries are scoped by a one-way credential hash and contain no secrets. The Mac validates `schemaVersion == 1` before displaying results, so a CLI contract mismatch fails closed per card.
- Exportable diagnostics are a test-build facility only. `TestDiagnosticLog` and the Settings export UI must remain guarded by `#if DEBUG`, so Release binaries contain neither log writes nor an export entry. The JSONL schema is typed and may contain only error codes, provider ids, status/meter counts, timestamps, and app/OS versions; never add raw stderr, response bodies, request headers, credentials, account identifiers, or filesystem paths. Files stay inside the current user's `~/Library/Logs/Vibe Usage/`, use owner-only permissions, and are shared only when that user explicitly exports them.
- Popover open → selected providers only: native Codex/Claude plus the Kimi/ZCode CLI adapters (60s debounce per provider). Neither path can prompt the user for another product's credentials.
...
- Display: the quota section always retains its selector, including when the user selects nothing. A cold `.noData` stays visible as 「正在读取订阅配额…」 while that provider's refresh is in flight (both legs now have real latency); two selected providers render side-by-side and one renders full-width. Cards accept provider-neutral meters, render at most two, and summarize the rest as 「另有 N 项」.
