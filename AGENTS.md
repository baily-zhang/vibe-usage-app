...
(`VibeUsage/Views/RateLimitCardView.swift`). The card is inside the dashboard's
vertical `ScrollView`, which clips its content, so a card-local tooltip was cut
off at the card's bottom edge; the panel root is the one layer above every
scroller, card, and following section. `QuotaTooltipPlacement` then positions the tooltip against the
...
- Display: every enabled product always gets its own card — the section never folds into one generic capability line, because a collapsed section reads as "the feature is off" exactly when the user needs to know why nothing is shown. Cards are a fixed 240pt wide (two of them plus the 8pt gap fill the 520px popover's 488pt content box exactly, so the familiar two-card row is unchanged) inside a two-column grid that grows downward: a third product starts a new row and the dashboard's own vertical scroll takes over, so the section never needs a nested horizontal scroller. Rows share the tallest card's height, so a provider with fewer meters still aligns with its neighbour. One product is a single 240pt card, and the header's selector always stays reachable, including when the user selects nothing (then, and only then, a hint line replaces the cards). Card states: `.ok` renders meters; a cold `.noData` stays visible as 「正在读取订阅配额…」 while that provider's refresh is in flight (both legs now have real latency); a settled `.noData` shows the reason available to it (see below); `.retryableError` (and `.unauthorized`) keep the provider card plus a retry button. A selected Cursor remains visible with 「已识别 Cursor · 等待官方配额接口」 even though it intentionally has no meters. Cards accept provider-neutral meters, show up to three, and fold anything beyond into a clickable 「另有 N 项」 footer line that expands/collapses the card in place — a folded window (OpenCode Go's monthly, Claude Max's model windows) is never unreachable.
