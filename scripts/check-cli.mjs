...
export function pinnedPackage() {
  ...
  assert.match(specifier || '', /^@vibe-cafe\/vibe-usage@\d+\.\d+\.\d+$/, 'CLI must pin an exact release');
  return specifier;
}
...
export function verifyPackage(packageRoot, specifier = pinnedPackage()) {
  ...
  assert.equal(manifest.version, version, 'Packed CLI does not match the Mac app pin');
...
export function checkCLI(localSource) {
  ...
  try {
    ...
    try {
      ...
      throw new Error(`无法获取 ${source}。正式打包前必须先发布并验证固定 CLI ${specifier}。`);
    }
