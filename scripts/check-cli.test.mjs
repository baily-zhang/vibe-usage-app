...
import { releasePackage, verifyPackage } from './check-cli.mjs';
...
test('the release specifier tracks latest and ignores local overrides', () => {
  const previous = process.env.VIBE_USAGE_CLI_PACKAGE;
  try {
    process.env.VIBE_USAGE_CLI_PACKAGE = '/temporary/unpublished-checkout';
    // Policy: normal builds run the published `latest`; compatibility is the
    // protocol check below, never a frozen version (see AGENTS.md).
    assert.equal(releasePackage(), '@vibe-cafe/vibe-usage@latest');
  } finally {
